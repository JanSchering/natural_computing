...
class NCA(t.nn.Module):
    ...
    def step(self, state:t.Tensor, rand_update_mask:t.Tensor, test=False) -> t.Tensor:
        """
    ...
    def forward(self, state:t.Tensor, test=False) -> List[t.Tensor]:
        """
        ...
        if test:
            for j in range(random.randint(self.min_steps, self.max_steps)):
                ...
                state = checkpoint(self.step, state, rand_update_mask, test)
            return [state]
        else:
            states = [state]
